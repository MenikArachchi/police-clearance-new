...
public class ApplicationStatusViewBusiness implements Serializable {
    ...
    public List<ApplicationVO> searchApplication(ApplicationStatusViewSearchCriteriaVO searchCriteriaVO, int userRole, long location, int userId) {
		Connection con = null;
		List<ApplicationVO> returnedApplicationVOs=new ArrayList<ApplicationVO>();
		try {
			con = DatabaseManager.getPOLDBConnection();
			con.setAutoCommit(false);
			returnedApplicationVOs=ApplicationStatusViewDAO.getInstance().searchApplicationForStatusView(searchCriteriaVO, con);
		...
		try {
			...
			if(!(applicationVO==null)){
				...
				if(StringUtils.isNotEmpty(applicationVO.getRecommendedOfficerName())){
					...
					List<CommentsVO> commentsVOs=CommentDAO.getInstance().getCommentListByTypeAndApplicationId(applicationId, PoliceEnumConstant.CommentType.ASP.toString(), con);
				...
				if(!(gridVO==null)){
					updateUserNamesInGridVOList(gridVO.getAddressStatusViewGridVOs());
	...
	private String getConstructedApprovalText(long applicationId, String hasApproved, String dateType,	Connection con) {
		if(StringUtils.isNotEmpty(hasApproved)){
			...
			List<ApplicationModifiedDatesVO> modifiedDatesVOs=ApplicationDAO.getInstance().getApplicationModifiedDateByTypeAndApplicationId(applicationId, dateType, con);
