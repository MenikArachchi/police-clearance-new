...
import lk.icta.police.business.OicClearenceBusiness;
...
public class OicClearenceAction extends ActionSupport implements SessionAware {

  private static final long serialVersionUID = 1L;
  private static final Logger LOGGER = Logger.getLogger(OicClearenceAction.class);
  ...
  public String viewOicClearence() {
    if (getUserTypeFromSession() == PoliceEnumConstant.UserType.OI.getCode()) {
      ...
      LOGGER.info("viewOicClearence ACTION||||||| earchCriteriaVO.getLimit() " + searchCriteriaVO.getLimit());
      LOGGER.info("viewOicClearence ACTION||||||| searchCriteriaVO.getMaxId() " + searchCriteriaVO.getMaxId());
      LOGGER.info("viewOicClearence ACTION||||||| searchCriteriaVO.getDefaultViewFromUi() "
          + searchCriteriaVO.getLockedRefresh());
      LOGGER.info("viewOicClearence ACTION||||||| searchCriteriaVO.getDefaultViewFromUi() "
          + searchCriteriaVO.getLockedRefresh());

      applicationList = OicClearenceBusiness.getInstance().searchApplication(searchCriteriaVO, userId);
  ...
  public String searchOicClearence() {
    ...
    if (getUserTypeFromSession() == PoliceEnumConstant.UserType.OI.getCode()) {
      ...
      LOGGER.info("userId ACTION||||||| " + userId);
      LOGGER.info("searchOicClearence ACTION||||||| earchCriteriaVO.getLimit() " + searchCriteriaVO.getLimit());
      LOGGER.info("searchOicClearence ACTION||||||| searchCriteriaVO.getMaxId() " + searchCriteriaVO.getMaxId());
      LOGGER.info("searchOicClearence ACTION||||||| searchCriteriaVO.getDefaultViewFromUi() "
          + searchCriteriaVO.getLockedRefresh());
      LOGGER.info("searchOicClearence ACTION||||||| searchCriteriaVO.getDefaultViewFromUi() "
          + searchCriteriaVO.getLockedRefresh());
  ...
  public String updateCertificateIssuanceOic() {
    ...
    if (!(clearenceVO == null)) {
      ...
      if (!(getUserTypeFromSession() == PoliceEnumConstant.UserType.OI.getCode())) {
        addActionError("You don't have permission to update Certificate Issuance as an OIC!");
        isValid = false;
      }
