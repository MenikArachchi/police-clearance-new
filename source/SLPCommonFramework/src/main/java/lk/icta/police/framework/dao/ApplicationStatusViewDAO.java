 ...
 *
 */
public class ApplicationStatusViewDAO {

	private static ApplicationStatusViewDAO instance = null;
	private static final Logger LOGGER = Logger.getLogger(ApplicationStatusViewDAO.class);
	
	public static ApplicationStatusViewDAO getInstance() {
		synchronized(ApplicationStatusViewDAO.class) {
			if (instance == null) {
				instance = new ApplicationStatusViewDAO();
			}
			return instance;
		}
	}
	
	private ApplicationStatusViewDAO(){
		
	}

			
	
	public List<ApplicationVO> searchApplicationForStatusView(ApplicationStatusViewSearchCriteriaVO criteriaVO,Connection conn) {
		List<ApplicationVO> applicationVOs = new ArrayList<ApplicationVO>();
		PreparedStatement pstm = null;
		ResultSet rst = null;
		try{
			//LOGGER.info(criteriaVO);
			//LOGGER.info(DBConstant.QUERY.SELECT_ALL_SEARCHED_APPLICATIONS_FOR_STATUS_VIEW);
			pstm = conn.prepareStatement(DBConstant.QUERY.SELECT_ALL_SEARCHED_APPLICATIONS_FOR_STATUS_VIEW);
			...
			pstm.setString(16, PoliceConstant.YES);
