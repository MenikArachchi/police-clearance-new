...
public class RequestClarificationBusiness {
    ...
    public RequestClarificationVO getRequestClarificationByReferenceNo(String referenceNo) {
		Connection con = null;
		RequestClarificationVO requestClarificationVO = new RequestClarificationVO();
		try {
			con = DatabaseManager.getPOLDBConnection();
			con.setAutoCommit(false);
			requestClarificationVO =  RequestClarificationDAO.getInstance().getRequestClarificationByReferenceNo(referenceNo, con);
